	...
	<div class="tank-section">
		<div class="upper-tanks">
			<div v-for="(color, index) in colors" :key="index" :style="{ backgroundColor: color }"
				:id="`upper-tank-${index}`" class="tank">
...
export default {
	...
	methods: {
		...
		decreaseTankLevels() {
			// randomly pick tank and item that is not black
			const idx = Math.floor(Math.random() * 4);
			const item = Math.floor(Math.random() * 5);

			// Decrease the height of the selected tank
			const tank = this.lowerTanks[idx][item];
			console.log(tank)
			if (tank['color'] === 'black') {
				return;
			}
			if (tank['height'] != '0%' && parseFloat(tank['height'].replace('%', '')) > 5) {
				...
				// Pick a new tank after the current one is empty
				this.startEmptyTank();
			}
		},
