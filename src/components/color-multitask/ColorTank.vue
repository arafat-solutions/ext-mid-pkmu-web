  ...
  <div>
    <div class="tank-section">
      ...
      <div class="line">
        <canvas
          ref="lineTankCanvas"
          :width="lineTankCanvasWidth"
          :height="lineTankCanvasHeight"
          style="border: 0px"
        ></canvas>
      </div>
      ...
      <div class="lower-tanks">
        <div
          v-for="(lowerTank, index) in lowerTanks"
          :key="index"
          class="tank"
          :id="`lower-tank-${index}`"
        >
          ...
          <div
            v-for="(tankItem, IdxColor) in lowerTank"
            :key="IdxColor"
            :style="{
      ...
      <div class="keyboard">
        <VirtualKeyboard
          @keyPress="handleVirtualKeyDown"
          :activeKeys="keySequence"
        />
      </div>
    </div>
  </div>
  ...
  methods: {
          ...
          [
            {
              ...
              height: "100%",
              status: "run",
              minimum_height: this.setMinimumHeight(),
            },
            {
              color: "black",
              height: "100%",
    ...
    startEmptyTank() {
      ...
      const findTankToEmpty = () => {
        ...
        if (
          selectedTank.height === "100%" &&
          selectedTank.color !== "black" &&
          selectedTank.status === "run"
        ) {
          setTimeout(() => {
            this.runningInterval("start-empty-tank");
          }, 1000);
    ...
    decreaseTankLevels(tankIndex, tankItem) {
      ...
      if (
        parseFloat(tank["height"].replace("%", "")) <=
        parseFloat(tank["minimum_height"].replace("%", ""))
      ) {
        tank["status"] = "stop";
        clearInterval(this.intervalStartEmptyTank);
        this.startEmptyTank();
    ...
    async fillTank(tankIndex, tankItem) {
      ...
      if (
        !this.lowerTanks[tankIndex] ||
        !this.lowerTanks[tankIndex][tankItem]
      ) {
        return;
      }
