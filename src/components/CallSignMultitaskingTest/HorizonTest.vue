...
export default {
  ...
  methods: {
    ...
    drawHorizon() {
      ...
      ctx.fillRect(
        config.x - config.width / 2,
        config.y + config.height / 2 - config.whiteBorderHeight,
        config.width * 2,
        config.whiteBorderHeight + config.height / 4
      );
    ...
    handleMouseEnter(event) {
      ...
      this.config.focusY = Math.max(
        ...
        Math.min(
          ...
          this.$refs.horizonCanvas.height
        )
      );
      this.drawVisual();
    },
    updateCorrectTime(isCorrect) {
      ...
      if (isCorrect && !this.isCurrentlyCorrect) {
        ...
        this.updateResults("horizon", {
          correct_time: currentTime - this.lastCorrectStartTime,
        });
    ...
    attemptReconnection() {
      ...
      console.log(
        `Attempting to reconnect (attempt ${this.reconnectionAttempts}/${this.maxReconnectionAttempts})`
      );
