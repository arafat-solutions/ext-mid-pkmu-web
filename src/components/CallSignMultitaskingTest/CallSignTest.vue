  ...
  methods: {
    ...
    clearCanvas() {
      this.ctx.clearRect(
        0,
        0,
        this.$refs.callsignCanvas.width,
        this.$refs.callsignCanvas.height
      );
    },
    drawText() {
    ...
    generateCallSign() {
      ...
      return (
        Array(2)
          .fill()
          .map(() => getRandomChar(letters))
          .join("") +
        Array(3)
          .fill()
          .map(() => getRandomChar(numbers))
          .join("")
      );
    },
    generateAngle() {
    ...
    speak(callsign) {
      ...
      this.speech = new SpeechSynthesisUtterance(
        `your callsign is: ${formattedCallsign}, and new heading is: ${formattedHeading}`
      );
    ...
    startTest() {
      ...
      this.generateAngle();
      const callsignToSpeak =
        Math.random() < this.matchesCall
          ? this.callsign
      ...
      // Then set the interval for repeated execution
