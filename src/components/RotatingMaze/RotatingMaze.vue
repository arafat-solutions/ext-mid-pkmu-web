...
export default {
  ...
  setup() {
    ...
    const setGridProperties = () => {
      ...
      if (gridSizeX.value % 2 == 0) gridSizeX.value += 1;
    ...
    const getInitialTargetPos = () => {
      ...
      const minDistance = Math.floor(
        Math.max(gridSizeX.value, gridSizeY.value) * 0.6
      ); // 60% of the larger grid dimension
      do {
        x = Math.floor(Math.random() * (gridSizeX.value - 2)) + 1;
        y = Math.floor(Math.random() * (gridSizeY.value - 2)) + 1;
      } while (
    ...
    const generateGrid = async () => {
      ...
      for (let i = 0; i < gridSizeY.value; i++) {
        ...
        for (let j = 0; j < gridSizeX.value; j++) {
          ...
          if ((i + j) % 2 == 0) class_name = "cell cell_1";
          else class_name = "cell cell_2";
      ...
      grid.value = new Array(gridSizeX.value)
        .fill(0)
        .map(() => new Array(gridSizeY.value).fill(0));
      ...
      placeToCell(targetPos.value[0], targetPos.value[1]).classList.add(
        "target"
      );
    };
      ...
      return new Promise((resolve) => {
        ...
        function sub_recursive_division(x_min, y_min, x_max, y_max) {
          if (y_max - y_min > x_max - x_min) {
            ...
            if ((x - x_min) % 2 == 1) x += randomInt(0, 2) == 0 ? 1 : -1;

            if ((y - y_min) % 2 == 0) y += randomInt(0, 2) == 0 ? 1 : -1;
        ...
        sub_recursive_division(
          0,
          0,
          grid.value.length - 1,
          grid.value[0].length - 1
        );
        timeouts.value.push(
          setTimeout(function () {
    ...
    const mazeGenerator = async () => {
      ...
      while (!solvableMaze && attempts < maxAttempts) {
        ...
        try {
          ...
          if (!solvableMaze) {
            console.log(
              `Generated maze has no solution. Attempt ${attempts}. Regenerating...`
            );
          } else {
      ...
      if (!solvableMaze) {
        console.error(
          "Failed to generate a solvable maze after maximum attempts"
        );
      }
    };
    ...
    const reconstructPath = () => {
      ...
      while (
        current[0] !== startPos.value[0] ||
        current[1] !== startPos.value[1]
      ) {
    ...
    const greedyBestFirst = async () => {
      ...
      while (frontier.length > 0 && !found.value) {
        frontier.sort(
          (a, b) => distance(a, targetPos.value) - distance(b, targetPos.value)
        );
        ...
        if (
          current[0] === targetPos.value[0] &&
          current[1] === targetPos.value[1]
        ) {
          found.value = true;
          break;
        }
        ...
        for (let neighbor of neighbors) {
          if (getNode(neighbor[0], neighbor[1]) === 0) {
            ...
            grid.value[neighbor[0]][neighbor[1]] = getDirection(
              current,
              neighbor
            );
          }
        ...
        if (!path.value) {
          if (nodeListIndex.value < nodeList.value.length) {
            ...
            if (!found.value) {
              ...
              path.value = true;
              placeToCell(startPos.value[0], startPos.value[1]).classList.add(
                "cell_path"
              );
            }
          }
    ...
    const updateConfigForNewDifficulty = (currentDifficulty) => {
      ...
      const configRotatingMaze = scheduleData.tests.find(
        (t) => t.testUrl === "rotating-maze-test"
      );
      const nextConfig = configRotatingMaze.configs.find(
        (c) => c.difficulty_level === nextDifficulty
      );

      if (!nextConfig) return false;

      const { rotation_frequency, size, number_of_question, difficulty_level } =
        nextConfig;

      config.value = {
        numberOfMaze: Number(number_of_question) ?? 10,
        rotationFrequency:
          ROTATION_FREQUENCY_VALUE[rotation_frequency] ??
    ...
    const movePlayer = async (direction) => {
      ...
      const currentIndex = pathList.value.findIndex(
        (cell) => cell[0] === x && cell[1] === y
      );
      const newIndex = pathList.value.findIndex(
        (cell) => cell[0] === newX && cell[1] === newY
      );
      ...
      quizMetrics.value.avgStepResponse = (
        (quizMetrics.value.avgStepResponse * (totalMoves.value - 1) +
          stepTime) /
        totalMoves.value
      ).toFixed(2);
      ...
      if (newX === targetPos.value[0] && newY === targetPos.value[1]) {
        ...
        if (!isTraining.value) {
          ...
          arrayMetrics.value.push({
            ...quizMetrics.value,
            completionTime: completeTime,
          });
          localStorage.setItem(
            "arrayMetrics",
            JSON.stringify(arrayMetrics.value)
          );
        }
    ...
    const initConfig = () => {
      ...
      const configMudah = configRotatingMaze.configs.find(
        (c) => c.difficulty_level === "Mudah"
      );
      const { rotation_frequency, size, number_of_question, difficulty_level } =
        configMudah;
