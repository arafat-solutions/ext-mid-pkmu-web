...
export default {
    ...
    setup(props, { emit }) {
        ...
        const getPillColor = () => {
            ...
            return (absAngle <= Math.PI / 4 || absAngle >= Math.PI * 7 / 4 || 
                   (absAngle > Math.PI / 4 && absAngle < Math.PI * 3 / 4)) ? 'red' : 'yellow';
        };
        ...
        const checkGamepadConnection = () => {
            ...
            for (let gamepad of gamepads) {
                if (gamepad) {
                    ...
                    } else if (gamepad.id === 'TWCS Throttle (Vendor: 044f Product: b687)') {
                        thruster = gamepad;
